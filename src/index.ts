  ...
  <script>
    ...
    const i18n = {
      ...
      translations: {
        zh: {
          ...
          tags: { title: '标签', allTags: '所有标签', addNew: '添加标签', createNew: '创建新标签', editTag: '编辑标签', tagName: '标签名称', noTags: '还没有标签。', searchPlaceholder: '搜索标签名称或别名', searchNoResults: '没有找到匹配的标签。', paginationSummary: '共 {total} 个标签，第 {page} / {totalPages} 页', previousPage: '上一页', nextPage: '下一页', createTag: '创建标签', updateTag: '更新标签', deleteConfirm: '确定要删除这个标签吗？' },
        ...
        en: {
          ...
          tags: { title: 'Tags', allTags: 'All Tags', addNew: 'Add New', createNew: 'Create New Tag', editTag: 'Edit Tag', tagName: 'Tag Name', noTags: 'No tags yet.', searchPlaceholder: 'Search tag name or slug', searchNoResults: 'No matching tags found.', paginationSummary: '{total} tags total, page {page} of {totalPages}', previousPage: 'Previous Page', nextPage: 'Next Page', createTag: 'Create Tag', updateTag: 'Update Tag', deleteConfirm: 'Are you sure you want to delete this tag?' },
    ...
    async function showPosts() {
        ...
        <div class="page-header">
          ...
          <div class="page-header-actions">
            <form id="post-search-form" class="search-form">
              <input type="search" id="post-search-input" class="search-input" value="\${escapeHtml(currentPostSearch)}" placeholder="\${escapeHtml(i18n.t('posts.searchPlaceholder'))}" autocomplete="off">
              <button type="submit" class="button button-secondary">\${i18n.t('common.search')}</button>
            </form>
      ...
      filter.addEventListener('change', (e) => {
        ...
        loadPosts(currentPostStatusFilter, currentPostSearch);
      });

      searchForm.addEventListener('submit', (e) => {
        e.preventDefault();
        clearTimeout(postSearchDebounceTimer);
        currentPostSearch = searchInput.value;
        currentPostStatusFilter = filter.value;
        loadPosts(currentPostStatusFilter, currentPostSearch);
      });
      ...
      searchInput.addEventListener('input', (e) => {
        ...
        postSearchDebounceTimer = setTimeout(() => {
          currentPostSearch = searchValue;
          currentPostStatusFilter = filter.value;
          loadPosts(currentPostStatusFilter, currentPostSearch);
        }, 300);
      });

      await loadPosts(currentPostStatusFilter, currentPostSearch);
    }

    async function loadPosts(status = getPostStatusFilter(), searchTerm = currentPostSearch) {
      try {
        currentPostStatusFilter = status;
        currentPostSearch = String(searchTerm || '').trim();
        const posts = sortAdminPosts(await fetchAllPosts(status, currentPostSearch));

        const container = document.getElementById('posts-list');
        if (posts.length === 0) {
          container.innerHTML = \`<div class="empty-state">\${currentPostSearch ? i18n.t('posts.searchNoResults') : (status === 'trash' ? '回收站里还没有文章。' : i18n.t('posts.noPostsYet'))}</div>\`;
          return;
        }
    ...
    window.showCreatePostModal = async function() {
      ...
      document.getElementById('create-post-form').addEventListener('submit', async (e) => {
        ...
        try {
          ...
          postData.featured_image_url = featuredImageUrl && featuredImageUrl.trim() ? featuredImageUrl.trim() : null;
    ...
    window.editPost = async function(id) {
      ...
      document.getElementById('edit-post-form').addEventListener('submit', async (e) => {
        ...
        try {
          ...
          // Include empty value so clearing the field removes the existing featured image.
          const featuredImageUrl = formData.get('featured_image_url');
          postData.featured_image_url = featuredImageUrl && featuredImageUrl.trim() ? featuredImageUrl.trim() : null;
    ...
    let tagSearchDebounceTimer;
    let tagListRequestId = 0;
    ...
    async function show标签() {
        ...
        <div class="page-header">
          ...
          <div class="page-header-actions">
            <form id="tag-search-form" class="search-form">
              <input type="search" id="tag-search-input" class="search-input" value="\${escapeHtml(currentTagSearch)}" placeholder="\${escapeHtml(i18n.t('tags.searchPlaceholder'))}" autocomplete="off">
              <button type="submit" class="button button-secondary">\${i18n.t('common.search')}</button>
            </form>
      ...
      tagSearchForm.addEventListener('submit', (e) => {
        ...
        currentTagPage = 1;
        load标签List(tagSearchInput.value, currentTagPage);
      });
      ...
      tagSearchInput.addEventListener('input', (e) => {
        ...
        tagSearchDebounceTimer = setTimeout(() => {
          currentTagPage = 1;
          load标签List(searchValue, currentTagPage);
        }, 300);
      });
    ...
    function formatTagPaginationSummary(totalItems, page, totalPages) {
      return i18n.t('tags.paginationSummary')
        .replace('{total}', totalItems)
        .replace('{page}', page)
        .replace('{totalPages}', totalPages);
    }

    function renderTagPagination(totalItems, page, totalPages) {
      if (totalItems <= 0) {
        return '';
      }

      const safeSummary = escapeHtml(formatTagPaginationSummary(totalItems, page, totalPages));
      const paginationActions = totalPages > 1 ? (() => {
        const previousLabel = escapeHtml(i18n.t('tags.previousPage'));
        const nextLabel = escapeHtml(i18n.t('tags.nextPage'));
        const previousDisabled = page <= 1 ? 'disabled' : '';
        const nextDisabled = page >= totalPages ? 'disabled' : '';

        return \`
          <div class="pagination-actions">
            <button type="button" class="button button-secondary pagination-button" onclick="loadTagPage(\${page - 1})" title="\${previousLabel}" aria-label="\${previousLabel}" \${previousDisabled}>&lsaquo;</button>
            <button type="button" class="button button-secondary pagination-button" onclick="loadTagPage(\${page + 1})" title="\${nextLabel}" aria-label="\${nextLabel}" \${nextDisabled}>&rsaquo;</button>
          </div>
        \`;
      })() : '';
      ...
      return \`
        <div class="pagination-bar">
          <div class="pagination-info">\${safeSummary}</div>
          \${paginationActions}
        </div>
      \`;
    }

    window.loadTagPage = function(page) {
      const nextPage = Math.max(1, Number(page) || 1);
      currentTagPage = nextPage;
      load标签List(currentTagSearch, currentTagPage);
    };

    async function load标签List(searchTerm = currentTagSearch, page = currentTagPage) {
      try {
        const requestId = ++tagListRequestId;
        const normalizedSearch = String(searchTerm || '').trim();
        const requestedPage = Math.max(1, Number(page) || 1);
        currentTagSearch = normalizedSearch;
        currentTagPage = requestedPage;
        const params = new URLSearchParams({
          per_page: String(TAGS_PER_PAGE),
        ...
        const totalItems = Number(response.headers.get('X-WP-Total') || tags.length || 0);
        const totalPages = Math.max(0, Number(response.headers.get('X-WP-TotalPages') || 0));

        if (requestId !== tagListRequestId) {
          return;
        }

        if (requestedPage > totalPages && totalPages > 0) {
          currentTagPage = totalPages;
          await load标签List(normalizedSearch, currentTagPage);
          return;
        }

        const container = document.getElementById('tags-list');
        if (tags.length === 0) {
          container.innerHTML = \`<div class="empty-state">\${normalizedSearch ? i18n.t('tags.searchNoResults') : i18n.t('tags.noTags')}</div>\`;
          return;
        }
        ...
        container.innerHTML = \`
          ...
          \${renderTagPagination(totalItems, currentTagPage, displayTotalPages)}
        \`;
      } catch (error) {
